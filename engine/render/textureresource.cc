...
namespace Render {
    ...
    /**
*/
    size_t PendingTextureLoads() { return loadingTasks.size(); }

    //------------------------------------------------------------------------------
    /**
*/
    void TextureResource::PollPendingTextureLoads() {
...
*/
    void TextureResource::Create() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            glTexImage2D(
                GL_TEXTURE_2D, 0, GL_RGBA, imageExtents.w, imageExtents.h, 0, GL_RGBA, GL_UNSIGNED_BYTE, buffer
            );
...
*/
    ...
    /**
*/
    TextureResourceId TextureResource::LoadTextureFromMemory(TextureLoadInfo const& info) {
        ...
        if (iid != InvalidImageId)
            ...
            byte pixel[] = {1, 1, 1};
            glTexImage2D(
                GL_TEXTURE_2D, 0, GL_RGB, createInfo.extents.w, createInfo.extents.h, 0, GL_RGB, GL_UNSIGNED_BYTE,
                &pixel
            );
        }
        ...
        std::packaged_task < ImageLoadResult() > task{
            [buffer = std::move(bufferCopy)]() {
                int w, h, channels;
                void* decompressed = stbi_load_from_memory(
                    (uchar*)buffer.data(), (int)buffer.size(), &w, &h, &channels, 0
                );
                assert(decompressed);
        ...
        loadingTasks.emplace_back(
            LoadTask{
                ...
                .complete = [
                    ...
                    if (img.channels == 3)
                        glTexImage2D(
                            GL_TEXTURE_2D, 0, sRGB ? GL_SRGB : GL_RGB, img.w, img.h, 0, GL_RGB, GL_UNSIGNED_BYTE,
                            img.data.get()
                        );
                    else if (img.channels == 4)
                        glTexImage2D(
                            GL_TEXTURE_2D, 0, sRGB ? GL_SRGB_ALPHA : GL_RGBA, img.w, img.h, 0, GL_RGBA,
                            GL_UNSIGNED_BYTE, img.data.get()
                        );
    ...
    /**
*/
    TextureResourceId TextureResource::LoadTexture(
        const char* path, MagFilter mag, MinFilter min, WrappingMode wrapModeS, WrappingMode wrapModeT,
        bool sRGB = false
    ) {
        ...
        std::cout << "Loaded " << w << "x" << h << " " << n << "BPP texture from file in : " << duration.count() <<
            " (ms)" << std::endl;
    ...
    /**
*/
    TextureResourceId TextureResource::LoadTextureFromMemory(
        std::string name, void* buffer, uint64_t bytes, ImageId imageId, MagFilter mag, MinFilter min,
        WrappingMode wrapModeS, WrappingMode wrapModeT, bool sRGB = false
    ) {
        ...
        auto start = std::chrono::high_resolution_clock::now();
        void* decompressed = stbi_load_from_memory(
            (uchar*)buffer, (int)bytes, (int*)&imageExtents.w, (int*)&imageExtents.h, (int*)&channels, 0
        );
        auto stop = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> duration = stop - start;
        std::cout << "Loaded " << imageExtents.w << "x" << imageExtents.h << " " << channels <<
            "BPP texture from memory in : " << duration.count() << " (ms)" << std::endl;
        glBindTexture(GL_TEXTURE_2D, imageHandle);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, (GLenum)min);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, (GLenum)mag);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, (GLenum)wrapModeS);
        ...
        if (channels == 3) {
            glTexImage2D(
                GL_TEXTURE_2D, 0, sRGB ? GL_SRGB : GL_RGB, imageExtents.w, imageExtents.h, 0, GL_RGB, GL_UNSIGNED_BYTE,
                decompressed
            );
        }
        else if (channels == 4) {
            glTexImage2D(
                GL_TEXTURE_2D, 0, sRGB ? GL_SRGB_ALPHA : GL_RGBA, imageExtents.w, imageExtents.h, 0, GL_RGBA,
                GL_UNSIGNED_BYTE, decompressed
            );
        }
...
*/
    ...
    /**
*/
    ImageExtents TextureResource::GetImageExtents(ImageId id) { return Instance()->imageExtents[id]; }

    //------------------------------------------------------------------------------
    /**
*/
    GLuint TextureResource::GetTextureHandle(TextureResourceId tid) { return Instance()->imageHandles[tid]; }

    //------------------------------------------------------------------------------
    /**
*/
    GLuint TextureResource::GetImageHandle(ImageId tid) { return Instance()->imageHandles[tid]; }
...
*/
    ImageId TextureResource::GetImageId(std::string name) {
        ...
        if (index != Instance()->imageRegistry.end()) { return (*index).second; }
    ...
    /**
*/
    TextureResourceId TextureResource::GetWhiteTexture() { return Instance()->whiteTexture; }

    //------------------------------------------------------------------------------
    /**
*/
    TextureResourceId TextureResource::GetBlackTexture() { return Instance()->blackTexture; }
...
*/
    ...
    /**
*/
    TextureResourceId TextureResource::GetDefaultNormalTexture() { return Instance()->defaultNormalTexture; }

    //------------------------------------------------------------------------------
    /**
*/
    void TextureResource::LoadTextureLibrary(std::string folder) { auto* instance = Instance(); }
} // namespace Render
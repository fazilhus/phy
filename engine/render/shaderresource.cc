...
*/
    ShaderProgramId ShaderResource::CompileShaderProgram(std::vector<ShaderResourceId> const& shaders) {
        ...
        for (auto shader: shaders) { glAttachShader(program, Instance()->shaders[shader]); }
    ...
    /**
*/
    GLuint ShaderResource::GetProgramHandle(ShaderProgramId programId) { return Instance()->programs[programId]; }
...
*/
    void ShaderResource::ReloadShaders() {
        ...
        for (size_t i = 0; i < Instance()->shaders.size(); i++) { glDeleteShader(Instance()->shaders[i]); }
        ...
        for (size_t i = 0; i < Instance()->programs.size(); i++) { glDeleteProgram(Instance()->programs[i]); }

        Instance()->programs.clear();
        Instance()->programShaders.clear();

        for (size_t i = 0; i < progs.size(); i++) { CompileShaderProgram(progs[i]); }
    }
} // namespace Render